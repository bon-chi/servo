...

{
    ...
    fn process_preorder(&self, node: N) -> RestyleResult {
        ...
        recalc_style_at::<N, LayoutContext, Self>(&self.context, self.root, node)
    }
...
fn construct_flows_at<'a, N: LayoutNode>(context: &'a LayoutContext<'a>, root: OpaqueNode, node: N) {
    // Construct flows for this node.
    {
        ...
        if nonincremental_layout || node.has_dirty_descendants() || node.restyle_damage() != N::ConcreteRestyleDamage::empty() {
